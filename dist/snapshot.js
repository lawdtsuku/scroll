...
export function validateSnapshot(p){
 ...
 if(p.format!=='scroll-trainer-snapshot'||![1,2].includes(p.version))throw Error('Choose a Scroll trainer snapshot, not a baseline, update or backup.');
...
export function exportSnapshot(s,owner){
 ...
 const p={format:'scroll-trainer-snapshot',version:2,exported_at:new Date().toISOString(),owner_character_id:owner,trainer:{character_id:c.character_id,name:c.name,level:c.level,xp_total:c.xp_total,resources:c.resources.map(r=>({id:r.id,label:s.resources.find(d=>d.id===r.id).label,current:r.current,max:r.max})),currency:{label:s.currency.label,amount:c.currency_amount},attributes:c.attributes.map(a=>({key:a.key,label:a.label,value:a.value,note:a.note||''})),ability_slots:(c.ability_slots||[]).map(slot=>({index:slot.index,name:slot.name,applications:(slot.applications||[]).map(a=>typeof a==='string'?{name:a,status:'not_recorded',cost:'not yet quantified',range:'not yet quantified',effect:'not yet quantified',notes:'Legacy reference; status not recorded in source.'}:{name:a.name,status:a.status,cost:a.cost,range:a.range,effect:a.effect,notes:a.notes})}))},roster:(s.roster||[]).filter(m=>m.owner_character_id===owner).map(m=>({member:structuredClone(m),progress:rosterProgress(m,s.roster_level_thresholds)})),inventory:c.inventory.map(i=>({item:i.item,qty:i.qty,consumable:i.consumable,notes:i.notes||''}))};
...
export function migrateSnapshot(p){validateSnapshot(p);if(p.version===2)return p;const next=structuredClone(p);next.version=2;for(const r of next.roster)if(r.member.attributes===undefined)r.member.attributes=[];return validateSnapshot(next);}
