...
export function validateSnapshot(p){
 exact(p,['format','version','exported_at','owner_character_id','trainer','roster','inventory',...(p.version===3?['roster_label','hide_owner_stats']:[])]);
 if(p.format!=='scroll-trainer-snapshot'||![1,2,3].includes(p.version))throw Error('Choose a Scroll trainer snapshot, not a baseline, update or backup.');
 ...
 if(p.version===3){text(p.roster_label,false,'roster_label');if(typeof p.hide_owner_stats!=='boolean')throw Error('hide_owner_stats must be boolean.');}
...
export function exportSnapshot(s,owner){
 ...
 const p={format:'scroll-trainer-snapshot',version:3,roster_label:s.roster_config?.label||'Roster',hide_owner_stats:s.roster_config?.hide_owner_stats===true,exported_at:new Date().toISOString(),owner_character_id:owner,trainer:{character_id:c.character_id,name:c.name,level:c.level,xp_total:c.xp_total,resources:c.resources.map(r=>({id:r.id,label:s.resources.find(d=>d.id===r.id).label,current:r.current,max:r.max})),currency:{label:s.currency.label,amount:c.currency_amount},attributes:c.attributes.map(a=>({key:a.key,label:a.label,value:a.value,note:a.note||''})),ability_slots:(c.ability_slots||[]).map(slot=>({index:slot.index,name:slot.name,applications:(slot.applications||[]).map(a=>typeof a==='string'?{name:a,status:'not_recorded',cost:'not yet quantified',range:'not yet quantified',effect:'not yet quantified',notes:'Legacy reference; status not recorded in source.'}:{name:a.name,status:a.status,cost:a.cost,range:a.range,effect:a.effect,notes:a.notes})}))},roster:(s.roster||[]).filter(m=>m.owner_character_id===owner).map(m=>({member:structuredClone(m),progress:rosterProgress(m,s.roster_level_thresholds)})),inventory:c.inventory.map(i=>({item:i.item,qty:i.qty,consumable:i.consumable,notes:i.notes||''}))};
 return validateSnapshot(p);
}
export function parseSnapshot(raw){if(typeof raw!=='string'||new TextEncoder().encode(raw).length>5*1024*1024)throw Error('Snapshot must be under 5 MB.');let p;try{p=JSON.parse(raw);}catch{throw Error('Invalid snapshot JSON. Choose the exported JSON file.');}return migrateSnapshot(p);}

export function migrateSnapshot(p){validateSnapshot(p);if(p.version===3)return p;const next=structuredClone(p);next.version=3;next.roster_label='Roster';next.hide_owner_stats=false;for(const r of next.roster)if(r.member.attributes===undefined)r.member.attributes=[];return validateSnapshot(next);}
